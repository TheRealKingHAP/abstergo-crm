...
function AddEmployeeForm({finishedSubmitFunction, intialData, userCancelFunction}: Props) {
    const [employee, setEmployee] = useState<Employee>(intialData ?? {
        email: '',
        apellido: '',
        area: '',
        nombre: '',
        ...
        <div>
            <form onSubmit={createEmployee} onReset={() => userCancelFunction ? handleUserCancel() : null}  className='mt-12 space-y-12 flex flex-col items-center'>
                ...
                <div className='flex space-x-12'>
                  <button type='submit' className='border-2 border-blue-500 text-gray-600 px-2 rounded-md'>Añadir empleado</button>
                  {userCancelFunction && <button type='reset' className='bg-gray-400 border-2 border-gray-400 font-medium text-white px-2 rounded-md'>Cancelar</button>}
                </div> 
                }
            </form>
        </div>
